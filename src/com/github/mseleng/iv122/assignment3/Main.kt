...
fun run(path: String) {
    ...
    SVG().lines(polygon(7, 200)).writeTo(fileWithName(path, 3, "polygon-7.svg"))
    print(" ✓ (assignment3/polygon-7.svg)\n-> Polygon (9 edges)......")
    SVG().lines(polygon(9, 200)).writeTo(fileWithName(path, 3, "polygon-9.svg"))
    print(" ✓ (assignment3/polygon-9.svg)\n-> Polygon (17 edges)......")
    SVG().lines(polygon(17, 100)).writeTo(fileWithName(path, 3, "polygon-17.svg"))
    print(" ✓ (assignment3/polygon-17.svg)\n-> Multistar (9 vertices)......")
    SVG().lines(multistar(9, 80)).writeTo(fileWithName(path, 3, "multistar-9.svg"))
    print(" ✓ (assignment3/multistar-9.svg)\n-> Multistar (5 vertices)......")
    SVG(150, 150).lines(multistar(5, 80)).writeTo(fileWithName(path, 3, "multistar-5.svg"))
    println(" ✓ (assignment3/multistar-5.svg)\n==========")

    //B
    print("-> Pentagram (turtle)......")
    SVG(200, 200).lines(anygram_turtle(5, 100)).writeTo(fileWithName(path, 3, "pentagram-turtle.svg"))
    print(" ✓ (assignment3/pentagram-turtle.svg)\n-> Nested square (20;[1:3];5])......")
    SVG(250, 250).lines(nestedSquare(200.0, Coordinates(30, 30), 20, Pair(1, 3), 5)).writeTo(fileWithName(path, 3, "nested-square.svg"))
    print(" ✓ (assignment3/nested-square.svg)\n-> Nested square (40;[1:7];5])......")
    SVG(250, 250).lines(nestedSquare(200.0, Coordinates(30, 30), 40, Pair(1, 7), 5)).writeTo(fileWithName(path, 3, "nested-square-2.svg"))
    print(" ✓ (assignment3/nested-square-2.svg)\n-> Nested square (40;[1:10];0])......")
    SVG(250, 250).lines(nestedSquare(200.0, Coordinates(30, 30), 40, Pair(1, 10), 0)).writeTo(fileWithName(path, 3, "nested-square-3.svg"))
    print(" ✓ (assignment3/nested-square-3.svg)\n-> Nested triangle ([25;5])......")
    SVG(550, 450).lines(nestedTriangle(500.0, Coordinates(270, 0), 25, 5)).writeTo(fileWithName(path, 3, "nested-triangle-2.svg"))
    print(" ✓ (assignment3/nested-triangle-2.svg)\n-> Nested triangle ([25;0])......")
    SVG(550, 450).lines(nestedTriangle(500.0, Coordinates(270, 0), 25, 0)).writeTo(fileWithName(path, 3, "nested-triangle.svg"))
    print(" ✓ (assignment3/nested-triangle.svg)\n-> Rounded flower.....")
    SVG(800, 800).lines(flower(100, Coordinates(300, 10))).writeTo(fileWithName(path, 3, "flower.svg"))
    print(" ✓ (assignment3/flower.svg)\n-> Lined circle.....")
    SVG(550).lines(linedCircle(550, 65)).writeTo(fileWithName(path, 3, "lined-circle.svg"))
    print(" ✓ (assignment3/lined-circle.svg)\n==========\n")
